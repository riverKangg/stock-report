...
def test_last_quarter_ratios_brkb(brkb_analyzer):
    ...
    assert df_last.loc[:, 'DividendYield'].values == 0
    assert df_last.loc[:, 'DividendPayoutRatio'].values is None
...
def test_get_financial_data_period_aapl(aapl_analyzer):
    start_date = '202201'
    end_date = '202303'
    df = aapl_analyzer.get_financial_data_period(start_date, end_date)
